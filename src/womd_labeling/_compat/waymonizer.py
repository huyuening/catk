...
class Waymonizer:
    def __init__(self, scenario, load_boundaries: bool = True) -> None:
        ...
        self._load_scenario(scenario, load_boundaries=load_boundaries)
    ...
    def _load_scenario(self, scenario, load_boundaries: bool = True) -> None:
        """
        ...
        # LOAD SCENARIO
        scenario_length = len(scenario.timestamps_seconds)
        ...
        for feature in scenario.map_features:
            ...
            if feature_data_type == "lane":
                if LaneType(feature.lane.type) in [LaneType.FREEWAY, LaneType.SURFACE_STREET]:
                    self.lanecenters[feature.id] = LaneCenter(
                        feature.id,
                        feature.lane,
                        tl_state_record=[WaymonicTLS.ABSENT for _ in range(scenario_length)],
                    )
            elif load_boundaries and feature_data_type in ["road_line", "road_edge"]:
    ...
    def _find_special_intersections(self, type: str) -> list[list[int]]:
        ...
        Args:
            type (str): The intersection evidence to use: signal, stop sign, or geometry.
        Returns:
        ...
        elif type == "stop_intersection":
            filter_criteria = self._stop_intersection_criteria
        elif type == "geometric_intersection":
            return connection_groups
        else:
            raise ValueError(f"Unsupported intersection type: {type}")
